...
info_box "Built with Agent Zero — AI Development Partner\n  https://www.agent-zero.ai | https://github.com/agent0ai/agent-zero"
...
if $TEST_MODE; then
    ...
    while true; do
        read -rp "Gateway IP (e.g., your router) [192.168.1.1]: " GATEWAY_IP
        GATEWAY_IP=${GATEWAY_IP:-192.168.1.1}
        if validate_ip "$GATEWAY_IP"; then break; else echo -e "${RED}Invalid IP format.${NC}"; fi
    done

    echo
    echo -e "${BOLD}DHCP Server${NC}"
    echo -e "  Enable the DHCP server on this Raspberry Pi?"
    echo -e "  ${YELLOW}WARNING:${NC} Only enable if your router's DHCP is disabled!"
    echo
    while true; do
        read -rp "Enable DHCP server? [Y/n]: " DHCP_ENABLED
        DHCP_ENABLED=${DHCP_ENABLED:-Y}
        if [[ $DHCP_ENABLED =~ ^[YyNn]$ ]]; then
            DHCP_ENABLED=$(echo "$DHCP_ENABLED" | tr '[:upper:]' '[:lower:]')
            if [[ $DHCP_ENABLED == "y" ]]; then DHCP_ENABLED="yes"; else DHCP_ENABLED="no"; fi
            break
        else
...
echo
if [[ $DHCP_ENABLED == "yes" ]]; then
    DHCP_SUMMARY="DHCP Range:   $DHCP_START - $DHCP_END"
else
    DHCP_SUMMARY="DHCP Server:  DISABLED"
fi
